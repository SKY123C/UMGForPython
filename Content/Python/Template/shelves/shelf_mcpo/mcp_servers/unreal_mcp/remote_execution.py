...
def test_main():
    remote_exec = RemoteExecution()  # 创建 RemoteExecution 实例
    remote_exec.start()  # 启动远程执行服务,
    import time
    time.sleep(1)
    # 获取远程节点 ID
    print("Available remote nodes:")
    for node in remote_exec.remote_nodes:
        # 打印节点的详细信息
        print(f"Node ID: {node['node_id']}, User: {node.get('user', 'N/A')}, Machine: {node.get('machine', 'N/A')}, Project: {node.get('project_name', 'N/A')}")
 
    if not remote_exec.remote_nodes:
        print("No remote nodes found. Make sure Unreal Engine is running with the Python plugin enabled.")
        remote_exec.stop()
        exit()
 
    # 选择第一个远程节点
    remote_node_id = remote_exec.remote_nodes[0]['node_id']
    print(f"Connecting to node: {remote_node_id}")
    ...
    result = remote_exec.run_command(command, exec_mode=MODE_EXEC_STATEMENT)
