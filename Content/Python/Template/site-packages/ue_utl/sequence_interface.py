...
class UnrealSequenceInterface(UnrealInterface):
    ...
    def find_bindings_by_generate_class(self, sequence: unreal.MovieSceneSequence, in_object):
        ...
        if unreal.MathLibrary.class_is_child_of(in_object.get_class(), unreal.Blueprint.static_class()):
            ...
            for binding in sequence.get_bindings():
                ...
                if not object_class:
                    continue
                if object_class == in_generated_class:
                    res.append(binding)
        else:
    ...
    @add_logger
    def remove_bindings_by_generate_class(self, sequence: unreal.MovieSceneSequence, in_object):
        res = []
        if unreal.MathLibrary.class_is_child_of(in_object.get_class(), unreal.Blueprint.static_class()):
            in_generated_class = editor_asset_system.load_blueprint_class(in_object.get_path_name())
            for binding in sequence.get_bindings():
            ...
            self.logger.error("传入的对象不是蓝图类型")
        for i in res:
            self.remove_binding(sequence, i)
    ...
    @add_logger
    @contextlib.contextmanager
    def open_sequence_scope(self, sequence: unreal.MovieSceneSequence):
        unreal.LevelSequenceEditorBlueprintLibrary.open_level_sequence(sequence)
        try:
            yield
        finally:
    ...
    @add_logger
    def remove_binding(self, sequence: unreal.MovieSceneSequence, root_binding_proxy: unreal.MovieSceneBindingProxy):
        # 广度遍历删除binding
        queue = [root_binding_proxy]
        queue1 = []
        while queue:
            binding_proxy = queue.pop()
            queue1.append(binding_proxy)
            child_possessables = binding_proxy.get_child_possessables()
            for i in child_possessables:
                queue.insert(0, i)
        for i in reversed(queue1):
            i.remove()
    
    @add_logger
    def iter_shot_track(self, in_sequence: unreal.MovieSceneSequence):
        track_list = in_sequence.find_tracks_by_type(unreal.MovieSceneCinematicShotTrack)
        unreal.LevelSequenceEditorBlueprintLibrary.open_level_sequence(in_sequence)
        if not track_list:
            self.logger.error("序列中未找到镜头轨道")
            return
        shot_track: unreal.MovieSceneCinematicShotTrack = track_list[0]
        for section in shot_track.get_sections():
            yield section
    
    @add_logger
    def find_or_add_track(self, binding_proxy: unreal.MovieSceneBindingProxy, in_track_class):
